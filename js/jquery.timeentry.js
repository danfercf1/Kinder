...
$.extend(TimeEntry.prototype, {
	...
	_doKeyDown: function(event) {
		...
		switch (event.keyCode) {
			case 9:  return true;/*return (event.shiftKey ?
	...
	_doKeyPress: function(event) {
		var aux_chr = (event.charCode == undefined || event.charCode=="")? event.keyCode : event.charCode;
		if(aux_chr!=9){
			var chr=String.fromCharCode(aux_chr);
			if (chr < ' ') {
				return true;
			}
			var inst = $.data(event.target, PROP_NAME);
	...
	_handleSpinner: function(event) {
        ...
        setTimeout(function(){  $.timeEntry._is_clicked=false;  }, 100);
	},
	...
	_handleKeyPress: function(inst, chr) {
		$.timeEntry._is_clicked=false;
		this._is_keypressed=true;
